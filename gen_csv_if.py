...
def extract_values(output_text):
    ...
    pattern_time = r"Run\s*ABC\s*on\s*the\s*ifg\s*dch\s*map\s*in\s*([\d.]+)\s*seconds"
...
directory = '/data/cchen/esyn2_base/E-syn2/Log_test_if_map/'
output_directory = '/data/cchen/esyn2_base/E-syn2/CSV_test_if_map/'
# 创建输出目录
...
for filename in os.listdir(directory):
    if filename.endswith('.txt'):
        ...
        # data = [[and_gate[i],area[i],lev[i], delay[i]] for i in range(len(lev))]  # 按索引对应组合数据
        # save_to_csv(data, csv_path)