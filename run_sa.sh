...
rewrite_circuit() {
    ...
    copy_file "circuit0.eqn" "../abc_old/circuit0_opt.eqn"
    change_dir "../abc_old"
    # alan
    #execute_command "./abc -c \"read_lib asap7_clean.lib;read_eqn circuit0_opt.eqn;if -K 6 -g -C 8;  strash; dch;ps;  map ;topo;  upsize; dnsize; stime;st;dch;ps;  map ;topo;  upsize; dnsize; stime;st;dch;ps;  map ;topo;  upsize; dnsize; stime; st;write_eqn circuit0_opt.eqn\"" 
     execute_command "./abc -c \"read_lib asap7_clean.lib;read_eqn circuit0_opt.eqn;if -K 6 -g -C 8;  strash; dch;ps;  map ;st;dch;ps;  map ;st;dch;ps;  st;write_eqn circuit0_opt.eqn\"" 
...
graph_to_equation() {
    ...
    if [[ "$pattern" == *"random"* ]]; then
        ...
        for file in ./*.eqn; do
            ...
            copy_file "$file" "../abc_old/opt_$index.eqn"
        done
...
run_abc() {
    ...
    copy_file "e-rewriter/circuit0.eqn" "abc_old/ori.eqn"
    set -- extraction-gym/tmp_*.v
    [ "$#" -eq 0 ] && { echo "No tmp_*.v found"; exit 1; }

    cp -- "$1" abc_old/output.v
    #python graph2eqn_ng.py
    change_dir "abc_old/"

    start_time_process_abc_original_cir=$(date +%s.%N)
  
    execute_command "./abc -c \"read_eqn ori.eqn;  if -K 6 -g -C 8; read_lib asap7_clean.lib; st;dch;ps;map;st;  st;dch;ps;map;st; dch;ps;map;st;dch;ps;  map ;topo;  upsize; dnsize; stime;\"" 

    echo -e "${YELLOW}<------------------------------The result after SA-delay tuning  ------------------>${RESET}"
    execute_command "./abc -c \" read_lib asap7_clean.lib;read -m output.v;stime\"" 
...
compare_circuits() {
    ...
    execute_command "./abc -c \"read_lib asap7_clean.lib;read -m output.v;st;cec ori.eqn\""
...
report_runtime() {
    ...
    echo -e "${GREEN}Run ABC on the original sop balancing flow completed in ${RED}$runtime_process_abc_original_cir${GREEN} seconds.${RESET}"
