...
for folder in sorted(os.listdir(source_dir)):
    ...
    if os.path.isdir(source_folder):
        ...
        for subfolder in ["class_map", "edgelist", "feature"]:
            ...
            # 处理 class_map 子目录下的 delay 目录
            if subfolder == "class_map":
                source_delay_folder = os.path.join(source_subfolder, "class_map/delay")
                for filename in sorted(os.listdir(source_delay_folder)):
                    source_file = os.path.join(source_delay_folder, filename)
            ...
            elif subfolder == "edgelist":
                source_edgelist_folder = os.path.join(source_subfolder, "edgelist")
                for filename in sorted(os.listdir(source_edgelist_folder)):
                    source_file = os.path.join(source_edgelist_folder, filename)
                    i = int(filename.split('_')[-1].split('.')[0])
                    target_prob_folder = os.path.join(target_dir, f"{folder}_{i}", "raw")
                    os.makedirs(target_prob_folder, exist_ok=True)
                    target_file = os.path.join(target_prob_folder, filename)
                    shutil.copy(source_file, target_file)
            elif subfolder == "feature":
                source_feature_folder = os.path.join(source_subfolder, "feature")
                for filename in sorted(os.listdir(source_feature_folder)):
                    source_file = os.path.join(source_feature_folder, filename)
                    i = int(filename.split('_')[-1].split('.')[0])
                    target_prob_folder = os.path.join(target_dir, f"{folder}_{i}", "raw")
                    os.makedirs(target_prob_folder, exist_ok=True)
                    target_file = os.path.join(target_prob_folder, filename)
