...
impl ForeignTrait2 for EGraph {
    fn to_dot(&self) -> String {
        self.to_graphviz().print(&mut PrinterContext::default())
    }

    fn to_svg_file(&self, path: impl AsRef<std::path::Path>) -> std::io::Result<()> {
    ...
    fn to_dot_file(&self, path: impl AsRef<std::path::Path>) -> std::io::Result<()> {
