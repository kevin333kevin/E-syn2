...
// #define ABC_MAX_STR 1048576 // 1MB buffer, adjust as needed
// extern void (*g_abc_output_callback)(const char* output);
// void set_abc_output_callback(void (*callback)(const char*));
////////////////////////////////////////////////////////////////////////
///                          INCLUDES                                ///
////////////////////////////////////////////////////////////////////////
...
extern void capture_abc_output(const char* output);

// Replace the existing Abc_Print function with this one
static inline void Abc_Print( int level, const char * format, ... )
{
    ...
    if ( level == ABC_ERROR )
        n = snprintf(buffer, sizeof(buffer), "Error: ");
    else if ( level == ABC_WARNING )
        n = snprintf(buffer, sizeof(buffer), "Warning: ");

    va_start( args, format );
    vsnprintf(buffer + n, sizeof(buffer) - n, format, args);
    va_end( args );

    // Call the Rust function to capture the output
    capture_abc_output(buffer);

    // If you still want to print to console, keep this line
    //printf("%s", buffer);
}
